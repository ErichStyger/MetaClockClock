    ...
    if (!boardHasError[i]) { /* ask board if it is still not idle */
      ...
      if (MATRIX_CommandHasBeenSentToBoard(i)) {
        ...
        if (res==ERR_OK) { /* no error */
          ...
          McuLog_error("Failed checking lastError (addr 0x%02x), resend `matrix exeq` command", addr);
...
static uint8_t PrintHelp(const McuShell_StdIOType *io) {
  ...
  McuShell_SendHelpStr((unsigned char*)"", (unsigned char*)"<md>: mode (cc, cw, sh), lowercase mode letter is with acceleration control for start/stop, otherwise e.g Cw or CW\r\n", io->stdOut);
#if PL_CONFIG_IS_MASTER
...
#define IGNORE_SOME_CLOCKS   (0) /* example code to ignore some clock positions, e.g. if they cause some problems and for debugging */
#if IGNORE_SOME_CLOCKS
static bool IgnoreCallback(int32_t x, int32_t y, int32_t z) {
  return (x==0 && y==0); /* ignore center clock which is at (0,0,*) */
}
#endif
