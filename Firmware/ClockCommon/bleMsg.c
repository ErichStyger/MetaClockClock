...
bool BLEMSG_HandleMessage(void) {
  ...
  if (BLEMSG_GetItem(&msg, 0)==pdPASS) {
    if (msg.type==BLE_MSG_TYPE_BUTTON_PRESSED) {
      switch(msg.u.button) {
      case 1: /* button '1' */
        McuUtility_strcpy(cmd, sizeof(cmd), (uint8_t*)"matrix hand enable all on");
        break;
      case 2: /* button '2' */
        McuUtility_strcpy(cmd, sizeof(cmd), (uint8_t*)"matrix hand enable all off");
        break;
      case 3: /* button '3' */
        McuUtility_strcpy(cmd, sizeof(cmd), (uint8_t*)"matrix hand enable all on");
        break;
