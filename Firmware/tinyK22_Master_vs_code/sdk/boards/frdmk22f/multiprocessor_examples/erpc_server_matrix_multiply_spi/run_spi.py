...
if __name__ == "__main__":
    ...
    argParser.add_argument('-b', '--bd', default='500000', help='Baud rate (default value is 500000)')
    argParser.add_argument('-g', '--csport', help='Chip select GPIO port number (see the board schematics)')
    argParser.add_argument('-p', '--cspin', help='Chip select GPIO pin number (see the board schematics)')
    ...
    try:
        # initialize LIBUSBSIOSPI transport layer
        transport = erpc.transport.LIBUSBSIOSPITransport(args.bd, args.csport, args.cspin, args.devidx)

        # run client with LIBUSBSIOSPI transport layer
        runClient(transport)
