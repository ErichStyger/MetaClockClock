...
void PD_PowerBoardControlInit(uint8_t port,
                              pd_handle pdHandle,
                              hal_gpio_handle_t powerGpioHandle,
                              hal_gpio_handle_t gpioExtPowerDetectHandle)
{
...
#if defined(PD_CONFIG_EXTERNAL_POWER_DETECTION_SUPPORT) && (PD_CONFIG_EXTERNAL_POWER_DETECTION_SUPPORT)
    HAL_GpioGetInput(powerControl->gpioExtPowerDetectHandle, &externalPowerState);
#endif

    if ((phyPowerPinCtrl.enSNK1)
#if defined(PD_CONFIG_EXTERNAL_POWER_DETECTION_SUPPORT) && (PD_CONFIG_EXTERNAL_POWER_DETECTION_SUPPORT)
        && (!externalPowerState)
#endif
    )
    {
        PD_NX20PExitDeadBatteryMode(port);
    }
    voltage = (vbusPower.minVoltage * 50U) | (((uint32_t)(vbusPower.maxVoltage * 50U)) << 16U);
    PD_Control(powerControl->pdHandle, PD_CONTROL_INFORM_VBUS_VOLTAGE_RANGE, &voltage);
    return kStatus_PD_Success;
