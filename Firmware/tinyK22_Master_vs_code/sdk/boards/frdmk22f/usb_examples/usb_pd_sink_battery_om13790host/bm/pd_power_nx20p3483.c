...
void PD_NX20PInit(uint8_t port, uint32_t source5VOcpCurrent, uint32_t sinkOvpVoltage)
{
    ...
    nx20pInstance = &s_nx20pInstances[port - 1U];
    portsNX20PConfigArray[port - 1U].nx20pI2cSrcClock = HW_I2CGetFreq(portsNX20PConfigArray[port - 1U].nx20pI2CMaster);
    nx20pInstance->nx20pSlaveAddress                 = portsNX20PConfigArray[port - 1U].nx20pSlaveAddress;

    /* initialize I2C */
    if (PD_I2cInit(&nx20pInstance->i2cHandle, portsNX20PConfigArray[port - 1U].nx20pI2CMaster,
                   portsNX20PConfigArray[port - 1U].nx20pI2cSrcClock,
                   (PD_I2cReleaseBus)portsNX20PConfigArray[port - 1U].nx20pI2cReleaseBus) != kStatus_PD_Success)
    {
        return;
    }
