...
status_t flash_mem_write(uint32_t address, uint32_t length, const uint8_t *buffer)
{
    ...
    while (length)
    {
        ...
        if ((buffer != &s_flash_section_program_info.buffer[s_flash_section_program_info.storedBytes]) && (storeBytes != 0U))
        {
            (void)memcpy((void *)&s_flash_section_program_info.buffer[s_flash_section_program_info.storedBytes], (const void *)buffer, (size_t)storeBytes);
        }
...
status_t flash_mem_flush(void)
{
    ...
    if (s_flash_section_program_info.storedBytes)
    {
        ...
        if ((length < alignedLength) && (length < sizeof(s_flash_section_program_info.buffer)))
        {
