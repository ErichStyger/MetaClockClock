...
static status_t dac_pcm186x_set_gpio_val(pcm186x_handle_t *handle, int gpio_nr, int val)
{
    ...
    switch (gpio_nr)
    {
        case 0:
            /* Set muxmode to gpio */
            ret = PCM186x_ModifyReg(handle, PCM186X_GPIO1_0_CTRL, PCM186X_GPIO0_FUNC, 0);
            if (ret != kStatus_Success)
                goto end;

            /* Set gpio as output */
            ret = PCM186x_ModifyReg(handle, PCM186X_GPIO1_0_DIR_CTRL, PCM186X_GPIO0_DIR, 0);
            if (ret != kStatus_Success)
                goto end;
        ...
        case 1:
            /* Set muxmode to gpio */
            ret = PCM186x_ModifyReg(handle, PCM186X_GPIO1_0_CTRL, PCM186X_GPIO1_FUNC, 0);
            if (ret != kStatus_Success)
                goto end;

            /* Set gpio as output */
            ret = PCM186x_ModifyReg(handle, PCM186X_GPIO1_0_DIR_CTRL, PCM186X_GPIO1_DIR, 4 << 4);
            if (ret != kStatus_Success)
                goto end;
        ...
        case 2:
            /* Set muxmode to gpio */
            ret = PCM186x_ModifyReg(handle, PCM186X_GPIO3_2_CTRL, PCM186X_GPIO2_FUNC, 0);
            if (ret != kStatus_Success)
                goto end;

            /* Set gpio as output */
            ret = PCM186x_ModifyReg(handle, PCM186X_GPIO3_2_DIR_CTRL, PCM186X_GPIO2_DIR, 4);
            if (ret != kStatus_Success)
                goto end;
        ...
        case 3:
            /* Set muxmode to gpio */
            ret = PCM186x_ModifyReg(handle, PCM186X_GPIO3_2_CTRL, PCM186X_GPIO3_FUNC, 0);
            if (ret != kStatus_Success)
                goto end;

            /* Set gpio as output */
            ret = PCM186x_ModifyReg(handle, PCM186X_GPIO3_2_DIR_CTRL, PCM186X_GPIO3_DIR, 4 << 4);
            if (ret != kStatus_Success)
                goto end;
    ...
    /* Set / clear gpio */
    ret = PCM186x_ModifyReg(handle, PCM186X_GPIO_IN_OUT, 1 << (gpio_nr + 4), val ? 1 << (gpio_nr + 4) : 0);
...
status_t PCM186x_SetFormat(pcm186x_handle_t *handle, uint32_t mclk, uint32_t sampleRate, uint32_t bitWidth)
{
    ...
    /* Set slave mode */
    ret = PCM186x_ModifyReg(handle, PCM186X_CLK_CTRL, PCM186X_MST_MODE, 0);
    if (ret != kStatus_Success)
        goto end;
    ...
    /* Set format */
    ret = PCM186x_ModifyReg(handle, PCM186X_PCM_CFG, PCM186X_FMT, fmt);
    if (ret != kStatus_Success)
        goto end;

    /* Set word length */
    ret = PCM186x_ModifyReg(handle, PCM186X_PCM_CFG, PCM186X_RX_WLEN | PCM186X_TX_WLEN, (wlen << 6) | (wlen << 2));
    if (ret != kStatus_Success)
        goto end;

    /* Power up analog */
    ret = PCM186x_ModifyReg(handle, PCM186X_POWER_CTRL, PCM186X_PWRDN, 0);
...
status_t PCM186x_Deinit(pcm186x_handle_t *handle)
{
    ...
    /* Power down analog */
    ret = PCM186x_ModifyReg(handle, PCM186X_POWER_CTRL, PCM186X_PWRDN, 1);
    if (ret != kStatus_Success)
        goto end;
