...
#ifndef _FSL_CODEC_WM8962_ADAPTER_H_
#define _FSL_CODEC_WM8962_ADAPTER_H_

#include "fsl_wm8962.h"

/*!
 * @addtogroup wm8962_adapter
...
{
    return HAL_CODEC_WM8962_Init(handle, config);
}
...
{
    return HAL_CODEC_WM8962_Deinit(handle);
}
...
{
    return HAL_CODEC_WM8962_SetFormat(handle, mclk, sampleRate, bitWidth);
}
...
{
    return HAL_CODEC_WM8962_SetVolume(handle, playChannel, volume);
}
...
{
    return HAL_CODEC_WM8962_SetMute(handle, playChannel, isMute);
}
...
{
    return HAL_CODEC_WM8962_SetPower(handle, module, powerOn);
}
...
{
    return HAL_CODEC_WM8962_SetRecord(handle, recordSource);
}
...
{
    return HAL_CODEC_WM8962_SetRecordChannel(handle, leftRecordChannel, rightRecordChannel);
}
...
{
    return HAL_CODEC_WM8962_SetPlay(handle, playSource);
}
 ...
 * @return kStatus_Success is success, else configure failed.
 */
static inline status_t HAL_CODEC_ModuleControl(void *handle, uint32_t cmd, uint32_t moduleData)
{
    return HAL_CODEC_WM8962_ModuleControl(handle, cmd, moduleData);
}
#endif
