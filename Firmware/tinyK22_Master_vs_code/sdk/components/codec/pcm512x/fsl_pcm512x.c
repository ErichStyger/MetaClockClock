...
status_t PCM512x_SetFormat(pcm512x_handle_t *handle, uint32_t mclk, uint32_t sampleRate, uint32_t bitWidth)
{
    ...
    if ((sampleRate != kPCM512x_AudioSampleRate44100Hz) && (sampleRate != kPCM512x_AudioSampleRate88200Hz) &&
        (sampleRate != kPCM512x_AudioSampleRate176400Hz) && (sampleRate != kPCM512x_AudioSampleRate48KHz) &&
        (sampleRate != kPCM512x_AudioSampleRate96KHz) && (sampleRate != kPCM512x_AudioSampleRate192KHz))
    {
        ret = kStatus_InvalidArgument;
        goto end;
    }
    ...
    ret = PCM512x_ModifyReg(handle, PCM512x_SYNCHRONIZE, PCM512x_RQSY, PCM512x_RQSY_RESUME);
    if (ret != kStatus_Success)
        goto end;

    /* Normal operation mode */
    ret = PCM512x_ModifyReg(handle, PCM512x_POWER, PCM512x_RQST, 0);
    if (ret != kStatus_Success)
        goto end;
...
status_t PCM512x_SetMute(pcm512x_handle_t *handle, bool isEnabled)
{
    ...
    if (isEnabled)
    {
        ret = PCM512x_ModifyReg(handle, PCM512x_MUTE, PCM512x_RQML | PCM512x_RQMR, PCM512x_RQML | PCM512x_RQMR);
    }
    else
    {
        ret = PCM512x_ModifyReg(handle, PCM512x_MUTE, PCM512x_RQML | PCM512x_RQMR, 0);
    }
...
status_t PCM512x_Init(pcm512x_handle_t *handle, const pcm512x_config_t *config)
{
    ...
    /* Reset codec */
    ret = PCM512x_WriteReg(handle, PCM512x_RESET, PCM512x_RSTM | PCM512x_RSTR);
    if (ret != kStatus_Success)
        goto end;
    ret = PCM512x_WriteReg(handle, PCM512x_RESET, 0);
    if (ret != kStatus_Success)
        goto end;

    /* Default to standby mode */
    ret = PCM512x_ModifyReg(handle, PCM512x_POWER, PCM512x_RQST, PCM512x_RQST);
    if (ret != kStatus_Success)
        goto end;
