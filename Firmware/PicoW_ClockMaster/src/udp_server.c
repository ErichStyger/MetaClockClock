...
#if PL_CONFIG_USE_SANTA
        if (McuUtility_strncmp(rx_buffer, "santa set led on", sizeof("santa set led on")-1)==0) { /* hard-coded command */
          bool handled = false;

          uint8_t res = Santa_ParseCommand((unsigned char*)"santa set led on", &handled, McuShell_GetStdio());
          if (res==ERR_OK && handled) {
            McuUtility_strcpy(test_response, sizeof(test_response), (unsigned char*)"OK!");
          } else {
            McuUtility_strcpy(test_response, sizeof(test_response), (unsigned char*)"not!");
          }
        }
#endif
        int err;
