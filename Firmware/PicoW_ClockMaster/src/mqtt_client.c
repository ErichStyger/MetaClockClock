...
static void MqttClient_connection_cb(mqtt_client_t *client, void *arg, mqtt_connection_status_t status) {
  const struct mqtt_connect_client_info_t *client_info = (const struct mqtt_connect_client_info_t*)arg;
  ...
  MQTT_CLIENT_CONNECTION_CALLBACK(client, arg, status); /* call user callback */
}
...
uint8_t MqttClient_Connect(void) {
  ...
  if (mqtt.mqtt_client!=NULL) {
    McuLog_trace("MQTT client already connected");
...
#if PL_CONFIG_USE_MININI
  McuMinINI_ini_gets(NVMC_MININI_SECTION_MQTT, NVMC_MININI_KEY_MQTT_BROKER, MQTT_CLIENT_CONFIG_BROKER, (char*)mqtt.broker, sizeof(mqtt.broker), NVMC_MININI_FILE_NAME);
  McuMinINI_ini_gets(NVMC_MININI_SECTION_MQTT, NVMC_MININI_KEY_MQTT_CLIENT, MQTT_CLIENT_CONFIG_CLIENT, (char*)mqtt.client_id, sizeof(mqtt.client_id), NVMC_MININI_FILE_NAME);
  McuMinINI_ini_gets(NVMC_MININI_SECTION_MQTT, NVMC_MININI_KEY_MQTT_USER, MQTT_DEFAULT_USER, (char*)mqtt.client_user, sizeof(mqtt.client_user), NVMC_MININI_FILE_NAME);
  McuMinINI_ini_gets(NVMC_MININI_SECTION_MQTT, NVMC_MININI_KEY_MQTT_PASS, MQTT_CLIENT_CONFIG_PASS, (char*)mqtt.client_pass, sizeof(mqtt.client_pass), NVMC_MININI_FILE_NAME);
  mqtt.doPublishing = McuMinINI_ini_getbool(NVMC_MININI_SECTION_MQTT, NVMC_MININI_KEY_MQTT_PUBLISH, MQTT_CLIENT_CONFIG_PUBLISH, NVMC_MININI_FILE_NAME);
#else
  McuUtility_strcpy(mqtt.broker, sizeof(mqtt.broker), (unsigned char*)MQTT_CLIENT_CONFIG_BROKER);
  McuUtility_strcpy(mqtt.client_id, sizeof(mqtt.client_id), (unsigned char*)MQTT_CLIENT_CONFIG_CLIENT);
  McuUtility_strcpy(mqtt.client_user, sizeof(mqtt.client_user), (unsigned char*)MQTT_DEFAULT_USER);
  McuUtility_strcpy(mqtt.client_pass, sizeof(mqtt.client_pass), (unsigned char*)MQTT_CLIENT_CONFIG_PASS);
  mqtt.doPublishing = MQTT_CLIENT_CONFIG_PUBLISH;
#endif

  McuLog_info("Connecting MQTT client to '%s'", mqtt.broker);
   /* resolve host name to IP address: */
  for (nofRetry=5; nofRetry>=0; nofRetry--) {
    if (DnsResolver_ResolveName((char*)mqtt.broker, &mqtt.addr, 5*1000)!=0) { /* use DNS to resolve name to IP address */
...
#if PL_CONFIG_USE_MININI
  McuMinINI_ini_puts(NVMC_MININI_SECTION_MQTT, NVMC_MININI_KEY_MQTT_BROKER, (char*)mqtt.broker, NVMC_MININI_FILE_NAME);
#endif
  return ERR_OK;
}
...
#if PL_CONFIG_USE_MININI
  McuMinINI_ini_puts(NVMC_MININI_SECTION_MQTT, NVMC_MININI_KEY_MQTT_CLIENT, (char*)mqtt.client_id, NVMC_MININI_FILE_NAME);
#endif
  return ERR_OK;
}
...
#if PL_CONFIG_USE_MININI
  McuMinINI_ini_puts(NVMC_MININI_SECTION_MQTT, NVMC_MININI_KEY_MQTT_USER, (char*)mqtt.client_user, NVMC_MININI_FILE_NAME);
#endif
  return ERR_OK;
}
...
#if PL_CONFIG_USE_MININI
  McuMinINI_ini_puts(NVMC_MININI_SECTION_MQTT, NVMC_MININI_KEY_MQTT_PASS, (char*)mqtt.client_pass, NVMC_MININI_FILE_NAME);
#endif
  return ERR_OK;
}
